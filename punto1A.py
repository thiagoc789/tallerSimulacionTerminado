...
for fileLine in archivo:
    palabraExtraida = re.findall(patron, fileLine)
    for palabra in palabraExtraida:
        palabraLimpia = re.sub('\W|\s|[(]|[)]|["]|["]', '', palabra)
        datos.append(len(palabraLimpia))
archivo.close()
datos = datos[:1000]
print("Numeros generados:  ", datos)
datosNormalizados = []
